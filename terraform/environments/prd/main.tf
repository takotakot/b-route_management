...
module "bigquery" {
  ...
  instantaneous_usages_table_id   = "instantaneous_usages"
  ...
  total_usages_table_id   = "total_usages"
...
module "pubsub" {
  ...
  # Match existing subscription settings
  subscription_max_delivery_attempts = 33
  enable_subscription_retry_policy   = true
  subscription_retry_minimum_backoff = "10s"
  subscription_retry_maximum_backoff = "600s"

  publishers = ["allUsers"] # Allowed in prd as per user request
}
