resource "null_resource" "build_and_push" {
  ...
  triggers = {
    ...
    build_command    = "gcloud builds submit ${path.module}/../../../${var.source_dir} --pack image=${var.region}-docker.pkg.dev/${var.project_id}/${var.repository_id}/${var.image_name}:latest --env GOOGLE_FUNCTION_TARGET=pubsub_to_bq --project ${var.project_id}"
  }

  provisioner "local-exec" {
    command = "gcloud builds submit ${path.module}/../../../${var.source_dir} --pack image=${var.region}-docker.pkg.dev/${var.project_id}/${var.repository_id}/${var.image_name}:latest,env=GOOGLE_FUNCTION_TARGET=pubsub_to_bq --project ${var.project_id}"
  }
}
